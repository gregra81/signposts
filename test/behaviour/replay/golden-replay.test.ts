...
function goldenCases(): GoldenCase[] {
  ...
  return readdirSync(GOLDEN)
    ...
    .map((name) => ({
      ...
      repo: (parseYaml(readFileSync(path.join(GOLDEN, name, "expected.yaml"), "utf8")) as { repo: string })
        .repo,
    }));
}

function portsFor(repo: string, repoRoot: string, applied: CommitInput[]): GraphPorts {
  return {
    ...
    neighbours: { async find() { return []; } },
    index: {
      async existingIds() { return new Set<string>(); },
      async isBootstrap() { return false; },
      async byId() { return undefined; },
    },
...
describe.skipIf(!ready)("golden-set replay", () => {
  it.each(cases)("$name replays from fixtures", async ({ transcript, repo }) => {
    const applied: CommitInput[] = [];
    const ports = portsFor(repo, EVAL_REPO, applied);
