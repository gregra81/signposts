...
export type CredentialSource = "ANTHROPIC_API_KEY" | "ANTHROPIC_AUTH_TOKEN" | "none";

/** Chain order per 08-models-and-credentials.md: ANTHROPIC_API_KEY checked before ANTHROPIC_AUTH_TOKEN. */
export function detectCredentialSource(env: {
  hasApiKey: boolean;
  hasAuthToken: boolean;
}): CredentialSource {
  if (env.hasApiKey) return "ANTHROPIC_API_KEY";
  if (env.hasAuthToken) return "ANTHROPIC_AUTH_TOKEN";
  return "none";
}
...
function credentialLine(facts: DoctorFacts): string {
  return facts.credential === "none"
    ? "credentials: none found (set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN)"
    : `credentials: ${facts.credential}`;
}
